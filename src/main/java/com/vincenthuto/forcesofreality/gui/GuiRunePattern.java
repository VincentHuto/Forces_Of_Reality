 ...
 * net.minecraft.client.resources.I18n; import net.minecraft.item.ItemStack;
 * import net.minecraft.util.ResourceLocation; import
 * net.minecraft.util.text.StringTextComponent; import
 * net.minecraft.util.text.TextFormatting; import
 * net.minecraftforge.api.distmarker.Dist; import
 * net.minecraftforge.api.distmarker.OnlyIn;
 *
 * @OnlyIn(Dist.CLIENT) public class GuiAdornmentPattern extends Screen { final
 * ResourceLocation texture = new ResourceLocation(ForcesOfReality.MOD_ID,
 * "textures/gui/rune_pattern.png"); private static final ResourceLocation
 * GUI_Chisel = new ResourceLocation( ForcesOfReality.MOD_ID +
 * ":textures/gui/chisel_station.png"); int guiWidth = 175; int guiHeight = 228;
 * int left, top; String title; String text;
 *
 ...
 * RecipeChiselStation recipeIn, String textIn) { super(titleComponent);
 * this.icon = iconIn; this.recipe = recipeIn; this.text = textIn;
 *
 * }
 *
 ...
 *
 * GlStateManager.pushMatrix(); for (int i = 0; i < buttonList.size(); i++) {
 * buttonList.get(i).renderButton(matrixStack, 0, 0, 10); }
 * GlStateManager.popMatrix();
 *
 * GlStateManager.pushMatrix(); { GlStateManager.translatef((width / 2) - 20,
 * centerY + 10, 10); GlStateManager.scalef(0.9f, 1, 1);
 * GlStateManager.translatef(-65f, 25, 0);
 *
 * // drawCenteredString(matrixStack, font, I18n.format(text), 175, 10, 10); //
 * Split String(text,x,y,wrapwidth,color) font.drawWordWrap(new
 * StringTextComponent(TextFormatting.BLACK + I18n.format(text)), 5, 101, 170,
 * 0); font.drawWordWrap(new StringTextComponent(TextFormatting.GOLD +
 * I18n.format(text)), 4, 100, 170, 0);
 *
 * } GlStateManager.popMatrix();
 *
 * GlStateManager.pushMatrix(); { // The 10 for the z translate draws the text
 * ON the book GlStateManager.translatef(((width / 2) -
 * font.getStringWidth(title) / 2) - 75, centerY + 50, 10);
 * drawString(matrixStack, font, TextFormatting.GOLD +
 * I18n.format(recipe.getOutput().getTranslationKey()), 0, 0, 8060954);
 *
 * } GlStateManager.popMatrix();
 *
 * GlStateManager.pushMatrix(); { GlStateManager.translatef(centerX, centerY,
 * 0); GlStateManager.translatef(3, 3, 0); GlStateManager.scalef(1.9f, 1.7f,
 * 1.9f); RenderHelper.enableStandardItemLighting();
 * Minecraft.getInstance().getItemRenderer().renderItemAndEffectIntoGUI(icon,
 * 36, 6); if (recipe.getInputs().size() == 1) {
 * Minecraft.getInstance().getItemRenderer()
 * .renderItemAndEffectIntoGUI(recipe.getInputs().get(0).getMatchingStacks()[0],
 * 36, 6); } else if (recipe.getInputs().size() == 2) {
 * Minecraft.getInstance().getItemRenderer()
 * .renderItemAndEffectIntoGUI(recipe.getInputs().get(0).getMatchingStacks()[0],
 * 10, 36); Minecraft.getInstance().getItemRenderer()
 * .renderItemAndEffectIntoGUI(recipe.getInputs().get(1).getMatchingStacks()[0],
 * 10, 60);
 *
 * }
 *
 ...
 * recipe.getActivatedAdornments().size(); k++) { if
 * (runeButtonArray[l][m].getId() == recipe.getActivatedAdornments().get(k)) {
