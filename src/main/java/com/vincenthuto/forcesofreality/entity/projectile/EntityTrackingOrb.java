...
public class EntityTrackingOrb extends ThrowableProjectile {
	public static EntityType<EntityTrackingOrb> TYPE = EntityInit.tracking_orb.get();

	private static final String TAG_TIME = "time";
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(EntityTrackingOrb.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(EntityTrackingOrb.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityTrackingOrb(EntityType<EntityTrackingOrb> type, Level world) {
		super(type, world);
	}

	public EntityTrackingOrb(Level world) {
		this(TYPE, world);
	}
	...
	public void tick() {
		...
		for (int i = 0; i < steps; i++) {
			...
			if (level.random.nextInt(steps) <= 1)
				level.addParticle(ParticleTypes.ASH, particlePos.x + (Math.random() - 0.5) * 0.4,
						particlePos.y + (Math.random() - 0.5) * 0.4, particlePos.z + (Math.random() - 0.5) * 0.4, 0, 0,
						0);
			level.addParticle(GlowParticleFactory.createData(new ParticleColor(200, 0, 200)),
