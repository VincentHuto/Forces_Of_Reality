...
public class EntityJudgement extends ThrowableProjectile {
	public static EntityType<EntityJudgement> TYPE = EntityInit.judgement.get();

	private static final String TAG_TIME = "time";
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(EntityJudgement.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(EntityJudgement.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityJudgement(EntityType<EntityJudgement> type, Level world) {
		super(type, world);
	}

	public EntityJudgement(Level world) {
		this(TYPE, world);
	}
	...
	protected void onHit(@Nonnull HitResult pos) {
			...
			if (!(block instanceof BushBlock) && !(block instanceof LeavesBlock))
				if (!level.isClientSide) {
					this.level.explode(this, this.getX(), this.getY() + this.getBbHeight() / 16.0F,
							this.getZ(), 4.0F, Explosion.BlockInteraction.NONE);
				}
		...
		case ENTITY: {
			if (((EntityHitResult) pos).getEntity() == getTargetEntity())
				if (!level.isClientSide) {
					getTargetEntity().addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 400, 2));
				}
			remove(RemovalReason.KILLED);
			break;
		}
	...
	public void tick() {
		...
		for (int i = 0; i < steps; i++) {
			level.addParticle(ParticleTypes.FLAME, particlePos.x, particlePos.y, particlePos.z, 0, 0, 0);
			if (level.random.nextInt(steps) <= 1)
				level.addParticle(ParticleTypes.ENCHANT, particlePos.x + (Math.random() - 0.5) * 0.4,
