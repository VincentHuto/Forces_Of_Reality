...
public class EntityStarStrike extends ThrowableProjectile {
	public static EntityType<EntityStarStrike> TYPE = EntityInit.star_strike.get();

	private static final String TAG_TIME = "time";
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(EntityStarStrike.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(EntityStarStrike.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityStarStrike(EntityType<EntityStarStrike> type, Level world) {
		super(type, world);
	}

	public EntityStarStrike(Level world) {
		this(TYPE, world);
	}
