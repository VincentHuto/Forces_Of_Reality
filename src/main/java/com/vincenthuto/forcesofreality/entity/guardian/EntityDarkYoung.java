	...
	private void shock(Entity target) {
		if (target != null) {
			...
			HLPacketHandler.sendLightningSpawn(this.position().add(0.5, 0.5, 0.5), speedVec, 64.0f,
					(ResourceKey<Level>) this.level.dimension(), ParticleColor.BLACK, 2, 10, 9, 0.2f);
			HLPacketHandler.sendLightningSpawn(this.position().add(0.5, 0.5, 0.5), speedVec, 64.0f,
					(ResourceKey<Level>) this.level.dimension(), ParticleColor.WHITE, 2, 10, 9, 0.2f);
			HLPacketHandler.sendLightningSpawn(this.position().add(0.5, 0.5, 0.5), speedVec, 64.0f,
					(ResourceKey<Level>) this.level.dimension(), ParticleColor.PURPLE, 2, 10, 9, 0.2f);
