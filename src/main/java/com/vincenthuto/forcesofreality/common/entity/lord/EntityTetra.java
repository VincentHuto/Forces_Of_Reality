...
import com.vincenthuto.forcesofreality.common.entity.projectile.EntityJudgement;
import com.vincenthuto.forcesofreality.common.entity.projectile.EntityStarStrike;
import com.vincenthuto.forcesofreality.common.entity.summon.EntityThrone;
...
public class EntityTetra extends Monster implements IEntityAdditionalSpawnData {
	...
	public void aiStep() {
		...
		this.destPos = Mth.clamp(this.destPos, 1.2F, 1.5F);
		if (!this.onGround()) {
	...
	public void repel(Level world, AABB effectBounds, double x, double y, double z) {
		...
		for (Entity ent : list) {
			if (!(ent instanceof EntityThrone)) {
				...
				double distance = p.distanceTo(t) + 0.1D;
				Vec3 r = new Vec3(t.x - p.x, t.y - p.y, t.z - p.z);
				ent.setDeltaMovement(r.x * 3 / distance, r.y * 3 / distance, r.z * 3 / distance);
...
//		}
		...
		world.addParticle(GlowParticleFactory.createData(new ParticleColor(250, 200, 0)),
				...
				center.y + (1 * 0.15) - 1.3 + HLParticleUtils.inRange(-0.2, 0.2),
				center.z + HLParticleUtils.inRange(-0.1, 0.1), 0, -0.05, 0);
		...
		if (this.deathTicks <= 0) {
			...
			if (this.tickCount % 10 - random.nextInt(10) > 20) {
				pullPlayer(new AABB(this.position().add(-15, -15, -15), this.position().add(15, 15, 15)),
			...
			if (!this.onGround()) {
				if (attackRoll % 100 * diffMult == 0) {
					this.summonHolyFlare(random.nextInt(1) + 3);
					playSound(SoundInit.ENTITY_SERAPHIM_FLARE.get(), 0.6F, 0.8F + (float) Math.random() * 0.2F);
				}
			}
