...
public class EntityWolfShot extends ThrowableProjectile {
	public static EntityType<EntityWolfShot> TYPE = EntityInit.wolf_shot.get();

	private static final String TAG_TIME = "time";
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(EntityWolfShot.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(EntityWolfShot.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityWolfShot(EntityType<EntityWolfShot> type, Level world) {
		super(type, world);
	}

	public EntityWolfShot(Level world) {
		this(TYPE, world);
	}
	...
	public void tick() {
		...
		for (int i = 0; i < steps; i++) {
			level().addParticle(ParticleTypes.ANGRY_VILLAGER, particlePos.x, particlePos.y, particlePos.z, 0, 0, 0);
