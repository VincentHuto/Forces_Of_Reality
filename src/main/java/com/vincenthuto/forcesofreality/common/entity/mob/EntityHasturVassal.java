...
public class EntityHasturVassal extends Monster {
	...
	class FleeGoal extends Goal {
		...
		public void tick() {
			...
			if (livingentity != null) {
				...
				BlockState blockstate = EntityHasturVassal.this.level()
						.getBlockState(BlockPos.containing(EntityHasturVassal.this.getX() + vector3d.x,
								EntityHasturVassal.this.getY() + vector3d.y,
								EntityHasturVassal.this.getZ() + vector3d.z));
				FluidState fluidstate = EntityHasturVassal.this.level()
						.getFluidState(BlockPos.containing(EntityHasturVassal.this.getX() + vector3d.x,
								EntityHasturVassal.this.getY() + vector3d.y,
								EntityHasturVassal.this.getZ() + vector3d.z));
	...
	public void aiStep() {
		if (this.isAlive()) {
			...
			if (this.isInWaterOrBubble()) {
				...
				this.setDeltaMovement(this.getDeltaMovement().add((this.random.nextFloat() * 2.0F - 1.0F) * 0.4F, 0.5D,
		...
		if (this.isInWaterOrBubble()) {
			...
			if (!this.level().isClientSide) {
				this.setDeltaMovement(this.randomMotionVecX * this.randomMotionSpeed,
						this.randomMotionVecY * this.randomMotionSpeed, this.randomMotionVecZ * this.randomMotionSpeed);
			}
			...
			this.squidPitch += (-((float) Mth.atan2(f1, vector3d.y)) * (180F / (float) Math.PI) - this.squidPitch)
					* 0.1F;
		} else {
	...
	public void setTarget(@Nullable LivingEntity pTarget) {
		net.minecraftforge.event.entity.living.LivingChangeTargetEvent changeTargetEvent = net.minecraftforge.common.ForgeHooks
				.onLivingChangeTarget(this, pTarget,
						net.minecraftforge.event.entity.living.LivingChangeTargetEvent.LivingTargetType.MOB_TARGET);
		if (!changeTargetEvent.isCanceled()) {
			this.setTarget(changeTargetEvent.getNewTarget());
		}
	}
	...
	private void squirtInk() {
		...
		for (int i = 0; i < 30; ++i) {
			Vec3 vector3d1 = this.rotateVector(
					new Vec3(this.random.nextFloat() * 0.6D - 0.3D, -1.0D, this.random.nextFloat() * 0.6D - 0.3D));
