...
import com.huto.forces_of_reality.containers.ContainerVirtuousEnchanter;
...
@OnlyIn(Dist.CLIENT)
public class GuiVirtuousEnchanter extends ContainerScreen<ContainerVirtuousEnchanter> {
	private static final ResourceLocation ENCHANTMENT_TABLE_BOOK_TEXTURE = new ResourceLocation(ForcesOfReality.MOD_ID,
			"textures/entity/virtuous_table_book.png");
	private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(ForcesOfReality.MOD_ID,
			"textures/gui/virtuous_enchanter.png");
	...
	public ContainerVirtuousEnchanter contain;

	public GuiVirtuousEnchanter(ContainerVirtuousEnchanter screenContainer, PlayerInventory inv,
	...
	public boolean mouseClicked(double mouseX, double mouseY, int p_231044_5_) {
		...
		for (int k = 0; k < 3; ++k) {
			...
			if (d0 >= 0.0D && d1 >= 0.0D && d0 < 108.0D && d1 < 19.0D
					&& this.container.enchantItem(this.minecraft.player, k)) {
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		int i = this.container.getGemAmount();
		...
		for (int j = 0; j < 3; ++j) {
			...
			if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, (double) mouseX, (double) mouseY) && k > 0) {
				...
				if (enchantment == null) {
					...
					list.add(new TranslationTextComponent("forge.container.enchant.limitedEnchantability")
					...
					if (this.minecraft.player.experienceLevel < k) {
						...
						if (i1 == 1) {
							...
							iformattabletextcomponent = new TranslationTextComponent("container.enchant.lapis.many",
									i1);
						}

						list.add(iformattabletextcomponent
								.mergeStyle(i >= i1 ? TextFormatting.GRAY : TextFormatting.RED));
						...
						if (i1 == 1) {
							...
							iformattabletextcomponent1 = new TranslationTextComponent("container.enchant.level.many",
									i1);
						}
	...
	public ContainerVirtuousEnchanter getContain() {
		return contain;
	}

	public void setContain(ContainerVirtuousEnchanter contain) {
		this.contain = contain;
	}

	@Override
	protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack, int mouseX, int mouseY) {
		this.font.func_243248_b(matrixStack, this.title, (float) this.titleX, (float) this.titleY, 7804791);

		if (contain.getEnchantingType() != null) {
			this.font.func_243248_b(matrixStack,
					new StringTextComponent(TextFormatting.ITALIC + contain.getEnchantingType().toString()),
					(float) this.titleX + 100, (float) this.titleY, 7804791);
		}
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX,
			int mouseY) {
		/*
		 * this.renderBackground(matrixStack); RenderSystem.color4f(1.0f, 1.0f, 1.0f,
		 * 1.0f); this.minecraft.getTextureManager().bindTexture(BACKGROUND_TEXTURE);
		 * int x = (this.width - this.xSize) / 2; int y = (this.height - this.ySize) /
		 * 2; this.blit(matrixStack, x, y, 0, 0, this.xSize, this.ySize);
		 */
		...
		matrixStack.translate((double) ((1.0F - f1) * 0.2F), (double) ((1.0F - f1) * 0.1F),
				(double) ((1.0F - f1) * 0.25F));
		...
		IVertexBuilder ivertexbuilder = irendertypebuffer$impl
				.getBuffer(MODEL_BOOK.getRenderType(ENCHANTMENT_TABLE_BOOK_TEXTURE));
		MODEL_BOOK.render(matrixStack, ivertexbuilder, 15728880, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
		irendertypebuffer$impl.finish();
		matrixStack.pop();
		RenderSystem.matrixMode(5889);
		RenderSystem.viewport(0, 0, this.minecraft.getMainWindow().getFramebufferWidth(),
		...
		int l = this.container.getGemAmount();
		...
		for (int i1 = 0; i1 < 3; ++i1) {
			...
			if (l1 == 0) {
				...
				ITextProperties itextproperties = EnchantmentNameParts.getInstance()
						.getGalacticEnchantmentName(this.font, i2);
				int j2 = 6839882;
				...
				this.font.drawStringWithShadow(matrixStack, s, (float) (k1 + 86 - this.font.getStringWidth(s)),
