package com.huto.forces_of_reality.render.tiles.teleporter;
...
public class RenderTeleporter extends TileEntityRenderer<TileEntityTeleporter> {
	...
	//	if (tileEntityIn.getTeleportPosition().isPresent()) {
		...
		if (tileEntityIn.getBlockState().getValues().get(FACING).toString().toUpperCase()
			...
			matrixStackIn.translate(0.5f, 2D, -1.15F);
		} else if (tileEntityIn.getBlockState().getValues().get(FACING).toString().toUpperCase()
				.equals(FaceDirection.WEST.toString())) {
			matrixStackIn.translate(-0.5f, 2D, -0.15F);
		} else if (tileEntityIn.getBlockState().getValues().get(FACING).toString().toUpperCase()
	...
	private void renderCube(TileEntityTeleporter tileEntityIn, float offset, float fIn, Matrix4f matrix,
		...
		this.renderFace(tileEntityIn, matrix, vertxBuilder, 1.0F, 1.0F, 1.85F, 0.0f, 0.0F, 1.0F, 1.0F, 0.0F, f1, f, f1,
				Direction.EAST);

		this.renderFace(tileEntityIn, matrix, vertxBuilder, 0.0F, 0.0F, 0.075F, 1.85F, 0.0F, 1.0F, 1.0F, 0.0F, f1, f, f,
				Direction.WEST);

		this.renderFace(tileEntityIn, matrix, vertxBuilder, 0.0F, 1.0F, 1.85F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, f, f1, f2,
				Direction.NORTH);
	...
	private void renderFace(TileEntityTeleporter tileEntityIn, Matrix4f matrix, IVertexBuilder vertextBuilder,
		...
		if (players.contains(player)) {
			for (PlayerEntity currentPlayer : players) {
				 ...
				 * .getBlockState(currentPlayer.getPosition().add(0, -1, 0)).getBlock() ==
				 * Blocks.QUARTZ_BLOCK) {
				 */
				if (tileEntityIn.shouldRenderFace(direction) && direction.equals(Direction.EAST)) {
					...
					/*
					 * //Left vertextBuilder.pos(matrix, 1, 1.85f, 0+1).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 1+2, 1.85f+alpha, 1+2*
					 * alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 1+2, 0,
					 * 1+2* alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 1,
					 * 0, 0+1).color(r, g, b, alpha).endVertex(); //Right vertextBuilder.pos(matrix,
					 * 1+2, 1.85f+alpha, 0-2* alpha).color(r, g, b, alpha).endVertex();
					 ...
					 * vertextBuilder.pos(matrix, 1+2, 0, 0-2* alpha).color(r, g, b,
					 * alpha).endVertex(); //Top vertextBuilder.pos(matrix, 1, 1.85f-0.1f,
					 * 0).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix,1+2.15f,
					 * 1.85f-0.1f+alpha, 0-2* alpha).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, 1+2.85f* alpha, 1.85f-0.1f+alpha,1+2*
					 * alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 1,
					 * 1.85f-0.1f, 1).color(r, g, b, alpha).endVertex(); //Bottom
					 * vertextBuilder.pos(matrix,1+2* alpha, 0.01f,1.85f+ alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix,1+2* alpha, 0.01f,
					 * -2*alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix,
					 * 1,0.3f, -0.5f).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix,
					 * 0, 0.3f, 0.5f).color(r, g, b, alpha).endVertex();
					 */
				}
				if (tileEntityIn.shouldRenderFace(direction) && direction.equals(Direction.WEST)) {
					...
					/*
					 * //Left vertextBuilder.pos(matrix, 0-2, 0-alpha, 0-2* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0, 0, 1-1).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0, 1.85f, 1-1).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0-2, 1.85f+alpha, 0-2*
					 * alpha).color(r, g, b, alpha).endVertex(); //Right vertextBuilder.pos(matrix,
					 * 0, 0, 0+1).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 0-2,
					 * 0-alpha, 1+2* alpha).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, 0-2, 1.85f+alpha, 1+2* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0, 1.85f, 0+1).color(r, g, b,
					 * alpha).endVertex(); //Top vertextBuilder.pos(matrix,0,1.85f-0.1f,1).color(r,
					 * g, b, alpha).endVertex(); vertextBuilder.pos(matrix,-2*
					 * alpha,1.85f-0.1f+alpha,3* alpha).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix,-2* alpha,1.85f-0.1f+alpha,-2* alpha).color(r, g,
					 * b, alpha).endVertex(); vertextBuilder.pos(matrix,0,1.85f-0.1f,0).color(r, g,
					 * b, alpha).endVertex(); //Bottom vertextBuilder.pos(matrix, 0, 0.31f,
					 * 0).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, -2f*alpha,
					 * 0.01f, -2*alpha).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, -2f*alpha, 0.01f, 1.85f+1.2f*alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0, 0.31f, 1).color(r, g, b,
					 * alpha).endVertex();
					 */
				...
				if (tileEntityIn.shouldRenderFace(direction) && direction.equals(Direction.NORTH)) {
					...
					/*
					 * //Left vertextBuilder.pos(matrix, 0-2* alpha, 1.85f+alpha, 0-2*
					 * alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 0,
					 * 1.85f, 1-1).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 0,
					 * 0, 1-1).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 0-2*
					 * alpha, 0-alpha, 0-2* alpha).color(r, g, b, alpha).endVertex();
					 * 
					 * //Right vertextBuilder.pos(matrix, 0+1, 1.85f, 0).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0+3* alpha, 1.85f+alpha, 1-3*
					 * alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 0+3*
					 * alpha, 0-alpha, 1-3* alpha).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, 0+1, 0, 0).color(r, g, b, alpha).endVertex();
					 * 
					 * //Top vertextBuilder.pos(matrix,-2*alpha*alpha, 1.75f+alpha,-2).color(r, g,
					 * b, alpha).endVertex(); vertextBuilder.pos(matrix,3*alpha,
					 * 1.75f+alpha,-2).color(r, g, b, alpha).endVertex();
					 ...
					 * 
					 * //Bottom vertextBuilder.pos(matrix, 3* alpha, 0.01f,-2* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix,-2* alpha* (alpha*1f),
					 * 0.01f,-2* alpha).color(r, g, b, alpha).endVertex();
				...
				if (tileEntityIn.shouldRenderFace(direction) && direction.equals(Direction.SOUTH)) {
					...
					/*
					 * //Left vertextBuilder.pos(matrix, 1-3*alpha, 0, 0+3*alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 1-1, 0, 1-0).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 1-1, 1.75f, 1-0).color(r, g,
					 * b, alpha).endVertex(); vertextBuilder.pos(matrix, 1-3*alpha, 1.75f+alpha,
					 * 0+3*alpha).color(r, g, b, alpha).endVertex();
					 * 
					 * //Right vertextBuilder.pos(matrix, 3*alpha, 0, 3* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix,3*alpha, 1.75F+alpha,3*
					 * alpha).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix,1, 1.75F,
					 * 1).color(r, g, b, alpha).endVertex(); vertextBuilder.pos(matrix, 1, 0,
					 * 1).color(r, g, b, alpha).endVertex();
					 * 
					 * 
					 * //Top vertextBuilder.pos(matrix,0, 1.5F,0).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, 1, 1.5F, 0).color(r, g, b, alpha).endVertex();
					 * vertextBuilder.pos(matrix, 3*alpha, 1.75F+alpha, 3* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix,-2*alpha*alpha, 1.75F+alpha,3*
					 * alpha).color(r, g, b, alpha).endVertex();
					 * 
					 * //Bottom vertextBuilder.pos(matrix, 3*alpha, 0.01f,3* alpha).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 1, 0.3f, 1).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix, 0, 0.3f, 1).color(r, g, b,
					 * alpha).endVertex(); vertextBuilder.pos(matrix,-2*alpha*alpha, 0.01f,3*
					 * alpha).color(r, g, b, alpha).endVertex();
					 */
