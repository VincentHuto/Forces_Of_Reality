...
import com.huto.forces_of_reality.objects.tileenties.util.EnumCrystalTypes;
import com.huto.forces_of_reality.objects.tileenties.util.IImportableTile;
...
import com.huto.forces_of_reality.recipes.RecipeHarmonizer;
...
public class TileEntityCrystalHarmonizer extends TileVibeSimpleInventory
		implements ITickableTileEntity, IImportableTile {
	...
	public int level = 1;
	public static EnumCrystalTypes resonantState;
	List<ItemStack> lastRecipe = null;
	RecipeHarmonizer currentRecipe;

	public TileEntityCrystalHarmonizer() {
		super(TileEntityInit.crystal_harmonizer.get());
	}
	...
	public void checkStructure() {
		...
		if (blockUnder == Blocks.SANDSTONE) {
			resonantState = EnumCrystalTypes.TOPAZ;
		} else if (blockUnder == Blocks.END_STONE) {
			resonantState = EnumCrystalTypes.AMETHYST;
		} else if (blockUnder == Blocks.IRON_ORE) {
			resonantState = EnumCrystalTypes.HEMATITE;
		} else if (blockUnder == Blocks.LAPIS_BLOCK) {
			resonantState = EnumCrystalTypes.SAPPHIRE;
		} else if (blockUnder == Blocks.OBSIDIAN) {
			resonantState = EnumCrystalTypes.ONYX;
		} else if (blockUnder == Blocks.REDSTONE_BLOCK) {
			resonantState = EnumCrystalTypes.RUBY;
		} else if (blockUnder == BlockInit.opal_block.get()) {
			resonantState = EnumCrystalTypes.OPAL;
		} else {
			resonantState = EnumCrystalTypes.NONE;
		}

	}

	public EnumCrystalTypes getResonantState() {
		return resonantState;
	}

	public RecipeHarmonizer getCurrentRecipe() {
		for (RecipeHarmonizer recipe_ : ModHarmonizerRecipes.harmonizerRecipes) {
			if (recipe_.matches(itemHandler,resonantState)) {
	...
	public void updateRecipe() {
		for (RecipeHarmonizer recipe : ModHarmonizerRecipes.harmonizerRecipes)
			if (recipe.matches(itemHandler, resonantState)) {
	...
	public void onActivated(PlayerEntity player, ItemStack wand) {
		...
		if (recipe != null && getResonantState() == recipe.getRecipeType()) {
			...
			if (vibes.getVibes() >= manaCost) {
				...
				currentRecipe = null;
				world.addBlockEvent(getPos(), BlockInit.crystal_harmonizer.get(), SET_COOLDOWN_EVENT, 60);
				world.addBlockEvent(getPos(), BlockInit.crystal_harmonizer.get(), CRAFT_EFFECT_EVENT, 0);
