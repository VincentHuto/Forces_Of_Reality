...
public class BlockSomnolentHopper extends ContainerBlock {
	...
	private static final VoxelShape BASE_SHAPE = VoxelShapes.combineAndSimplify(INPUT_MIDDLE_SHAPE,
			IHopper.INSIDE_BOWL_SHAPE, IBooleanFunction.ONLY_FIRST);
	private static final VoxelShape DOWN_SHAPE = VoxelShapes.or(BASE_SHAPE,
	...
	private static final VoxelShape NORTH_SHAPE = VoxelShapes.or(BASE_SHAPE,
	...
	private static final VoxelShape WEST_SHAPE = VoxelShapes.or(BASE_SHAPE,
	...
	public BlockSomnolentHopper(AbstractBlock.Properties properties) {
		super(properties);
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new TileEntitySomnolentHopper();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		if (stack.hasDisplayName()) {
			...
			if (tileentity instanceof TileEntitySomnolentHopper) {
				((TileEntitySomnolentHopper) tileentity).setCustomName(stack.getDisplayName());
			}
		}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (worldIn.isRemote) {
			...
			TileEntity tileentity = worldIn.getTileEntity(pos);
			if (tileentity instanceof TileEntitySomnolentHopper) {
				player.openContainer((TileEntitySomnolentHopper) tileentity);
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (!state.isIn(newState.getBlock())) {
			TileEntity tileentity = worldIn.getTileEntity(pos);
			if (tileentity instanceof TileEntitySomnolentHopper) {
				InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntitySomnolentHopper) tileentity);
	...
	public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
		TileEntity tileentity = worldIn.getTileEntity(pos);
		if (tileentity instanceof TileEntitySomnolentHopper) {
			((TileEntitySomnolentHopper) tileentity).onEntityCollision(entityIn);
		}
