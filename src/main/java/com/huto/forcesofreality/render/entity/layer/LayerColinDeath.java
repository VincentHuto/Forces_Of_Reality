...
public class LayerColinDeath extends LayerRenderer<EntityColin, ModelColin> {
	...
	protected static ResourceLocation TEXTURE = new ResourceLocation(ForcesOfReality.MOD_ID, "textures/entity/colin/modelcolin.png");
	static Random random = new Random(432L);

	private final ModelColin model = new ModelColin();
	private static final RenderType field_229054_i_ = RenderType.getEntityDecal(TEXTURE);

	public LayerColinDeath(IEntityRenderer<EntityColin, ModelColin> entityRendererIn) {
		super(entityRendererIn);
	}

	@Override
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, EntityColin entityIn,
		...
		if (entityIn.deathTicks > 0) {
			float f2 = (float) entityIn.deathTicks / 300.0F;
		...
		if (entityIn.deathTicks > 0) {
			float f5 = ((float) entityIn.deathTicks + partialTicks) / 350.0F;
	...
	private static void func_229061_a_(IVertexBuilder vertextBuilder, Matrix4f matrix4f, int alpha) {
		vertextBuilder.pos(matrix4f, 0.0F, 0.0F, 0.0F).color(122, 122, 122, alpha).endVertex();
		vertextBuilder.pos(matrix4f, -0.3F, -0.3F, -.3F).color(122, 122, 122, alpha).endVertex();
	}

	private static void func_229060_a_(IVertexBuilder vertextBuilder, Matrix4f matrix4f, float y, float x) {
		vertextBuilder.pos(matrix4f, -constant * x, y, -0.3F * x).color(0, 255, 0, 0).endVertex();
	}

	private static void func_229062_b_(IVertexBuilder vertextBuilder, Matrix4f matrix4f, float y, float x) {
		vertextBuilder.pos(matrix4f, constant * x, y, -0.3F * x).color(255, 0, 0, 0).endVertex();
	}

	private static void func_229063_c_(IVertexBuilder vertextBuilder, Matrix4f matrix4f, float y, float z) {
		vertextBuilder.pos(matrix4f, 0.0F, y, -0.3f * z).color(0, 0, 255, 0).endVertex();
	}
}
