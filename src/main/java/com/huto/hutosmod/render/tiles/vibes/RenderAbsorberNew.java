...
public class RenderAbsorberNew extends TileEntityRenderer<TileEntityAbsorber> {
	...
	public RenderAbsorberNew(TileEntityRendererDispatcher rendererDispatcherIn) {
	...
	public void render(TileEntityAbsorber te, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn,
		...
		List<BlockPos> connectedNodes = te.getLinkedBlocks();
		for (BlockPos targetpos : connectedNodes) {
			if (canAdd(te.getPos(), targetpos)) {
				lasers.put(te.getPos(), targetpos);
			}
		}
		drawLasers(matrixStackIn, lasers, r, g, b);
