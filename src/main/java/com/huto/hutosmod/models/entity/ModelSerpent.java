...
@OnlyIn(Dist.CLIENT)
public class ModelSerpent extends Model {
	...
	protected float[][] undulationCycle = new float[][] { { 45F, -45F, -45F, 0F, 45F, 45F, 0F, -45F },
	...
	public void renderSerpent(Entity parEntity, float parTime, float parSwingSuppress, float par4, float parHeadAngleY,
	...
	private void setRotationAngles(float parTime, float parSwingSuppress, float par4, float parHeadAngleY,
			float parHeadAngleX, float par7, Entity parEntity) {
		// animate if moving
		updateDistanceMovedTotal(parEntity);
		cycleIndex = (int) ((getDistanceMovedTotal(parEntity) * CYCLES_PER_BLOCK) % undulationCycle.length);
		// DEBUG
		System.out.println("ModelSerpent setRotationAngles(), distanceMoved =" + getDistanceMovedTotal(parEntity)
	...
	// spin methods are good for testing and debug rotation points and offsets in
	// the model
