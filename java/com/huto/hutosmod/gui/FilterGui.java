...
public class FilterGui extends ContainerScreen<FilterContainer> {
	...
	protected void init() {
		...
		Button.IPressable slotClick = new Button.IPressable() {
			...
			public void onPress(Button button) {
				Minecraft.getInstance().playerController.sendEnchantPacket(container.windowId,
						((SlotButton) button).slot);
		...
		addButton(new SwitchButton(guiLeft + 80, guiTop + 8, "hutosmod.whitelist",
				((container.getFilterOpts() & 1) > 0),
				(button) -> ((SwitchButton) button).state = (container.setFilterOpts(container.getFilterOpts() ^ 1)
						& 1) > 0));
		addButton(new SwitchButton(guiLeft + 80, guiTop + 8 + 18, "hutosmod.nbtdata",
				((container.getFilterOpts() & 2) > 0),
				(button) -> ((SwitchButton) button).state = (container.setFilterOpts(container.getFilterOpts() ^ 2)
						& 2) > 0));
		addButton(new SwitchButton(guiLeft + 80, guiTop + 8 + 54, "hutosmod.autopickup", container.getPickup(),
				(button) -> ((SwitchButton) button).state = container.togglePickup()));

	}

	private ResourceLocation GUI = new ResourceLocation(HutosMod.MOD_ID, "textures/gui/filter_gui.png");;
		...
		for (Widget button : buttons) {
			if (button.isMouseOver(x, y) && button instanceof SlotButton)
				if (!container.itemHandler.filter.getStackInSlot(((SlotButton) button).slot).isEmpty())
					renderTooltip(matrixStack, container.itemHandler.filter.getStackInSlot(((SlotButton) button).slot),
							x, y);
		}
	}
	...
	class SlotButton extends Button {
		...
		public void renderButton(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
			...
			if (container.itemHandler.filter != null && container.itemHandler.filter.getStackInSlot(slot) != null
					&& !container.itemHandler.filter.getStackInSlot(slot).isEmpty()) {
	...
	class SwitchButton extends Button {
		...
		private ResourceLocation off = new ResourceLocation(HutosMod.MOD_ID, "textures/gui/switch_off.png");
		private ResourceLocation on = new ResourceLocation(HutosMod.MOD_ID, "textures/gui/switch_on.png");
