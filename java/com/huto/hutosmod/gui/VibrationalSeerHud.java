...
public class VibrationalSeerHud extends Screen {
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		IVibrations vibeCap = player.getCapability(VibrationProvider.VIBE_CAPA).orElseThrow(IllegalArgumentException::new);
	...
	/*	PacketHandler.CHANNELVIBES.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player),
				new VibrationPacketServer(vibes));*/
		IVibrations vibeCap = player.getCapability(VibrationProvider.VIBE_CAPA).orElseThrow(IllegalArgumentException::new);
		//PacketHandler.CHANNELVIBES.send( new VibrationPacketClient());
	//	PacketHandler.INSTANCE.sendToServer(new PacketGetVibes(vibes, "com.huto.hutosmod.render.vibesViewerHud", "vibes"));
	...
	//	GL11.glScalef(1.0f, 1.20f, 1);
		...
		//fr.drawString(null, "vibes Value", -fr.getStringWidth(d.format(vibes)) - 49, -29, 0x000000);
		//fr.drawString("vibes Value", -fr.getStringWidth(d.format(vibes)) - 50, -29, 0x2F9AFF);
		GL11.glPopMatrix();
		// Digits
		GL11.glPushMatrix();
