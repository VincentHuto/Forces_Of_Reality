...
public class EntityCorruptNote extends ThrowableEntity {
	public static EntityType<EntityCorruptNote> TYPE = EntityInit.corrupt_note.get();

	private static final String TAG_TIME = "time";
	private static final DataParameter<Boolean> EVIL = EntityDataManager.createKey(EntityCorruptNote.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> TARGET = EntityDataManager.createKey(EntityCorruptNote.class,
			DataSerializers.VARINT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityCorruptNote(EntityType<EntityCorruptNote> type, World world) {
		super(type, world);
	}

	public EntityCorruptNote(World world) {
		this(TYPE, world);
	}
	...
	public void tick() {
		...
		for (int i = 0; i < steps; i++) {
			...
			if (world.rand.nextInt(steps) <= 1)
				world.addParticle(ParticleTypes.NOTE, particlePos.x + (Math.random() - 0.5) * 0.4,
