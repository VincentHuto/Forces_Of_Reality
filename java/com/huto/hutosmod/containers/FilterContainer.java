...
import com.huto.hutosmod.network.FilterMessage;
...
public class FilterContainer extends Container {
	...
	public boolean canInteractWith(PlayerEntity playerIn) {
		if (slotID == -106)
			return playerIn.getHeldItemOffhand().getItem() instanceof ItemRuneBinder; // whoops guess you can...
		return playerIn.inventory.getStackInSlot(slotID).getItem() instanceof ItemRuneBinder;
	}

	public RuneBinderItemHandler itemHandler;
	private int slotID;
	private PlayerEntity player;
	private ItemStack item;

	@SuppressWarnings("rawtypes")
	public static final ContainerType type = new ContainerType<>(FilterContainer::new)
			.setRegistryName("rune_binder_filter_container");
	...
	public FilterContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory,
			PlayerEntity playerEntity) {
		super(type, windowId);

		item = findRuneBinder(playerEntity);
		...
		if (tmp instanceof RuneBinderItemHandler) {
			itemHandler = (RuneBinderItemHandler) tmp;
			itemHandler.load();
			player = playerEntity;
		} else {
			playerEntity.closeScreen();
	...
	public int setFilterOpts(int newOpts) {
		...
		item.setTag(nbt);
		if (player.getEntityWorld().isRemote)
			PacketHandler.RUNEBINDER.sendToServer(new FilterMessage(newOpts));
		return newOpts;
	}
