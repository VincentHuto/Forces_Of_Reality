...
public class FXLightning extends Particle {
	...
	public FXLightning(ClientWorld world, Vector3 sourcevec, Vector3 targetvec, float ticksPerMeter, long seed,
	...
	public void renderBolt(int pass, boolean inner) {
		...
		for (FXLightningSegment rendersegment : segments) {
			...
			wr.pos(endvec.x - diff2.x, endvec.y - diff2.y, endvec.z - diff2.z).tex(0.5f, 0).lightmap(0xF0, 0xF0)
					.color(r, g, b, a).endVertex();
			wr.pos(startvec.x - diff1.x, startvec.y - diff1.y, startvec.z - diff1.z).tex(0.5f, 0).lightmap(0xF0, 0xF0)
					.color(r, g, b, a).endVertex();
			wr.pos(startvec.x + diff1.x, startvec.y + diff1.y, startvec.z + diff1.z).tex(0.5f, 1).lightmap(0xF0, 0xF0)
					.color(r, g, b, a).endVertex();
			wr.pos(endvec.x + diff2.x, endvec.y + diff2.y, endvec.z + diff2.z).tex(0.5f, 1).lightmap(0xF0, 0xF0)
					.color(r, g, b, a).endVertex();

			if (rendersegment.next == null) {
				Vector3 roundend = rendersegment.endPoint.point.add(rendersegment.diff.normalize().multiply(width));

				wr.pos(roundend.x - diff2.x, roundend.y - diff2.y, roundend.z - diff2.z).tex(0, 0).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
				wr.pos(endvec.x - diff2.x, endvec.y - diff2.y, endvec.z - diff2.z).tex(0.5f, 0).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
				wr.pos(endvec.x + diff2.x, endvec.y + diff2.y, endvec.z + diff2.z).tex(0.5f, 1).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
				wr.pos(roundend.x + diff2.x, roundend.y + diff2.y, roundend.z + diff2.z).tex(0, 1).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
			}
			...
			if (rendersegment.prev == null) {
				...
				wr.pos(startvec.x - diff1.x, startvec.y - diff1.y, startvec.z - diff1.z).tex(0.5f, 0)
						.lightmap(0xF0, 0xF0).color(r, g, b, a).endVertex();
				wr.pos(roundend.x - diff1.x, roundend.y - diff1.y, roundend.z - diff1.z).tex(0, 0).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
				wr.pos(roundend.x + diff1.x, roundend.y + diff1.y, roundend.z + diff1.z).tex(0, 1).lightmap(0xF0, 0xF0)
						.color(r, g, b, a).endVertex();
				wr.pos(startvec.x + diff1.x, startvec.y + diff1.y, startvec.z + diff1.z).tex(0.5f, 1)
						.lightmap(0xF0, 0xF0).color(r, g, b, a).endVertex();
			}
		}
	}
	...
	private void fractal(int splits, double amount, double splitChance, double splitLength, double splitAngle) {
		...
		for (FXLightningSegment segment : oldSegments) {
			...
			for (int i = 0; i < splits; i++) {
				FXLightningSegment next = new FXLightningSegment(newpoints[i], newpoints[i + 1], segment.light,
				...
				if (i != 0 && rand.nextFloat() < splitChance) {
					...
					Vector3 diff = next.diff.rotate((rand.nextFloat() * 0.66F + 0.33F) * splitAngle, splitrot)
							.multiply(splitLength);

					splitCount++;
					splitParents.put(splitCount, next.splitNo);

					FXLightningSegment split = new FXLightningSegment(newpoints[i],
							new FXLightningBoltPoint(newpoints[i + 1].basepoint, newpoints[i + 1].offsetvec.add(diff)),
							segment.light / 2F, next.segmentNo, splitCount);
					split.prev = prev;
	...
	private float rayTraceResistance(Vector3 start, Vector3 end, float prevresistance) {
		...
		RayTraceContext ctx = new RayTraceContext(start.toVec3D(), end.toVec3D(), RayTraceContext.BlockMode.OUTLINE,
				RayTraceContext.FluidMode.NONE, viewer);
	...
	private static Vector3 getRelativeViewVector(Vector3 pos) {
		...
		return new Vector3((float) renderEntity.getPosX() - pos.x,
				(float) renderEntity.getPosY() + renderEntity.getEyeHeight() - pos.y,
