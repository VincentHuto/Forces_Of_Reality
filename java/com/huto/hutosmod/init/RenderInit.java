...
public class RenderInit extends RenderType {
	...
	public static final RenderType TRANSLUCENT = makeType("testTrans", DefaultVertexFormats.POSITION_COLOR_TEX, 7, 256, false,
			true, State.getBuilder().texture(new TextureState(orbTrans, false, false))
					.transparency(CRUMBLING_TRANSPARENCY).writeMask(COLOR_DEPTH_WRITE).build(false));
	
	public static final RenderType HASTURTRANS = makeType("hasturTrans", DefaultVertexFormats.POSITION_COLOR_TEX, 7, 256, false,
			true, State.getBuilder().texture(new TextureState(hastur_pylon, false, false))
					.transparency(CRUMBLING_TRANSPARENCY).writeMask(COLOR_DEPTH_WRITE).layer(field_239235_M_).build(false));
	
	public static final RenderType SPHEREIN = makeType("sphereTrans", DefaultVertexFormats.POSITION_COLOR_TEX, 7, 256, false,
			true, State.getBuilder().texture(new TextureState(orbTrans, false, false))
					.transparency(CRUMBLING_TRANSPARENCY).writeMask(COLOR_DEPTH_WRITE).build(false));
	public static final RenderType ENDTRANS = makeType("end_trans", DefaultVertexFormats.POSITION_COLOR_TEX, 7, 256, false,
			true, State.getBuilder().texture(new TextureState(end_trans, false, false))
					.transparency(CRUMBLING_TRANSPARENCY).writeMask(COLOR_DEPTH_WRITE).build(false));
	...
	public static RenderType getTranslucent(ResourceLocation locationIn) {
		return makeType("my_type", DefaultVertexFormats.POSITION_COLOR, 7, 256, false, true,
				State.getBuilder().transparency(TRANSLUCENT_TRANSPARENCY).writeMask(COLOR_DEPTH_WRITE)
						.texture(new TextureState(locationIn, false, false)).build(false));
	}
